...
namespace NEA_Project
{
	public partial class DB_Load_Page : Form
	{
		...
		//When called, this function should get all the files ralated to the current user and display them
		//onto a table using a ListView form element.
			...
			switch (selectedFileType)
			{
				...
				case "image":
					try
					{
						...
						//Set its value to the returned bitmap from decompress image.
						...
						//As we are loading an image file, it is likely that the user will want to download,
						//said image file. And therfore we can set the combo box option to be "text file
		...
		{	
			...
			//Firstly we need to re create the table generated by the compression algorithm.
			//This table will include each character and its associated binary code.
			//The compression string exists in the form "'a10'b01'" where the ' marks the start of a new letter
			//followed by the binary sequence for that letter.
			
			//This loop seperates the data in the compression string into a linked list (acting as the table).
			for (int i = 0; i < len; i++)
			{ 
				//If the current character is ', then we know that the next letter is in the next position.
				if (compressionString[i] == '\'')
				{
					...
					//Now we have the character, we can fetch the associated binary sequence.
					//To do this I add each character after the letter to another string called temp sequence.
					//This continues until another ' has been reached. As when another ' is found we know that the
					...
					//Add the new data to the linked list.
			...
			//Convert the linked list acting as a table to an array with a fixed size.
			//We only need the data structure to be dynamic during the compression string deconstruction,
			//as the amount of unqiue letters cannot be known.
			...
			//Now we have our table stored as an array, we can sipmply substitute the compressed binary with each
			//characters binary equivilent.
			foreach (char binary in text)
			{
				//Each bit is added to a string.
				tempBinary += binary;
				
				//This string is then checked to see if it matches any binary sequence.
				for (int i = 0; i < letterAndSequence.Length; i++)
				{
					//If it does then we can add the associated letter to the decompressed text string.
					if (tempBinary == letterAndSequence[i][1])
					{
						decompressedText += letterAndSequence[i][0];
						
						//The temp binary string is also reset.
						tempBinary = "";
					}
				}
			}
		...
		private Bitmap addPixels(Bitmap bm, int amountToAdd, char colourChar)
		{
			...
			for (int i = 0; i < amountToAdd; i++)
			{
				...
				//When the end of a row has been reached, the y is incremented so that we can 
				//continue to draw pixels on the next row.
				if (x == bm.Width)
				{
					//The x is set to 0 as we have moved onto the start of a new row.
					x = 0;
					y++;
				}
			}
			
			//Return the original bitmap with the added pixels.
			return bm;
		}
		...
		private int binaryToDenary(string binary)
		{
			...
			//Due to how this function works, the input binary must be flipped before it is converted.
			//The following loop flips the order of the string.
